...
class Model(nn.Module):
    ...
    def model_enc(self, x, edge_index):
        ...
        self.emb = torch.utils.checkpoint.checkpoint(
            self.gat_layer, h, edge_index, use_reentrant=False
        )

        if self.emb.shape[0] > 30000:
            batch_size = 5000
            n = self.emb.shape[0]
            s_parts = []
            for i in range(0, n, batch_size):
                end = min(i + batch_size, n)
                s_parts.append(torch.sigmoid(self.emb[i:end] @ self.emb.T))
            s_ = torch.cat(s_parts, dim=0)
        else:
    ...
    def forward(self, seq1, adj, idx_train, idx_test, sparse=False):
        ...
        edge_index = torch.tensor(np.array(edge_index)).T.to(seq1.device)
