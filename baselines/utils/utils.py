...
def normalize_adj(adj):
    ...
    # 度的开方的倒数（对称归一化公式里要用），避免孤立节点(度为0)的除零警告
    d_inv_sqrt = np.power(rowsum, -0.5, where=rowsum>0, out=np.zeros_like(rowsum, dtype=float)).flatten()
    # 建对角矩阵
...
def load_mat(dataset, train_rate=0.3, val_rate=0.1, file_path=None):
    ...
    if file_path is None:
        ...
        if dataset.endswith('.pkl'):
            ...
            # 向上查找 datasets 文件夹（类似 utils_tam.py 的实现）
            def find_datasets_dir(start_path, max_levels=8):
                ...
                for level in range(max_levels + 1):
                    ...
                    if os.path.exists(datasets_path) and os.path.isdir(datasets_path):
                        print(f"找到 datasets 文件夹: {datasets_path} (向上 {level} 层)")
                        return datasets_path
                    parent_path = os.path.dirname(current_path)
                    if parent_path == current_path:
                        break
                    current_path = parent_path
                print(f"警告: 向上查找 {max_levels} 层未找到 datasets 文件夹")
                return None
            
            # 从当前文件所在目录开始向上查找
            utils_dir = os.path.dirname(os.path.abspath(__file__))
            datasets_dir = find_datasets_dir(utils_dir)
            
            if datasets_dir is None:
                # 如果没找到，使用默认路径
                datasets_dir = os.path.join(os.path.dirname(utils_dir), 'datasets')
    ...
    if len(abnormal_idx) == 0:
        ...
        num_train_abnormal = max(1, int(num_abnormal * train_rate)) if train_rate > 0 else 0
        num_val_abnormal = max(1, int(num_abnormal * val_rate)) if val_rate > 0 else 0
