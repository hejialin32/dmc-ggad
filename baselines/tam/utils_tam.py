...
def load_mat(dataset):
    """Load .mat or .npz dataset."""
    # Try multiple possible dataset paths - 与utils.py保持一致
    possible_paths = [
    ...
    if data is None:
        ...
        raise FileNotFoundError(f"Dataset {dataset} not found in any of the expected locations: {possible_paths}")
    
    # Handle .npz files (like tolokers.npz)
    if file_ext == '.npz':
        print(f"Loading .npz file: {dataset}")
        ...
        adj = sp.lil_matrix((num_node, num_node))
        for edge in edges:
            adj[edge[0], edge[1]] = 1
            adj[edge[1], edge[0]] = 1
        adj = adj.tocsr()
        ...
        # Features
        feat = sp.lil_matrix(node_features)
        
        # Labels
        ano_labels = node_labels
        str_ano_labels = None
        attr_ano_labels = None
        
        # Use predefined masks (use the first fold)
        idx_train = np.where(train_masks[0])[0].tolist()
        idx_val = np.where(val_masks[0])[0].tolist()
        idx_test = np.where(test_masks[0])[0].tolist()
        
        all_idx = list(range(num_node))
        
        # Sample labeled normal nodes from training set
        ...
        print(f'Loaded {dataset}: {num_node} nodes, {edges.shape[0]} edges')
...
def adj_to_dgl_graph(adj):
    """Convert adjacency matrix to networkx format (placeholder for compatibility)."""
...
def graph_nsgt(dis_array, adj):
    ...
    # 处理dis_array_u中没有非零元素的情况
    non_zero_elements = dis_array_u[dis_array_u != 0]
    if non_zero_elements.numel() > 0:
        mean_dis = non_zero_elements.mean()
    else:
        mean_dis = 0.0  # 设置默认值
    ...
    for i in range(row):
        ...
        if node_index.shape[0] != 0:
            ...
            min_dis = mean_dis
            # 确保min_dis不是NaN
            if not torch.isnan(min_dis) and max_dis > min_dis:
...
def max_message(feature, adj_matrix, normal_label_idx):
    ...
    feature[torch.isinf(feature) | torch.isnan(feature)] = 0
    sim = torch.mm(feature, feature.T) * adj_matrix
    sim[torch.isinf(sim) | torch.isnan(sim)] = 0
    row_sum = adj_matrix.sum(0)
    r_inv = row_sum.pow(-1).flatten()
    r_inv[torch.isinf(r_inv)] = 0.
    r_inv[torch.isnan(r_inv)] = 0.
    message = sim.sum(1) * r_inv
    message[torch.isinf(message) | torch.isnan(message)] = 0
    # 处理message中所有值相同的情况
    if message.max() == message.min():
        message = torch.zeros_like(message)
    else:
        message = (message - message.min()) / \
            (message.max() - message.min() + 1e-8)
    return -message[normal_label_idx].sum(), message
